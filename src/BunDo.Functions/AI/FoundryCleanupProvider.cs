...
public sealed class FoundryCleanupProvider(HttpClient http, TokenCredential credential, Uri endpoint, string deployment) : ICleanupProvider
{
    public const string Instructions = """
        ...
        Do not translate, invent details, resolve dates, create checklist items, reorder or schedule tasks.
    ...
    public async Task<CleanupProposal> GenerateAsync(string title, string? description, CancellationToken ct)
    {
        ...
        request.Content = JsonContent.Create(new {
            ...
            input = JsonSerializer.Serialize(new { title, description }), max_output_tokens = 4096,
    ...
    public static CleanupProposal ParseResponse(byte[] bytes)
    {
        try
        {
            ...
            if (names.Length != 4 || names.Distinct().Count() != 4 ||
                names.Except(["title", "description", "language", "needsReview"]).Any()) throw new CleanupProviderException("INVALID_OUTPUT");
            var proposal = new CleanupProposal(value.GetProperty("title").GetString()!,
                ...
                value.GetProperty("needsReview").GetBoolean());
