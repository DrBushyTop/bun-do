...
internal object SharedTaskActions {
    ...
    val kinds = setOf("ClaimTask", "UnclaimTask", "CompleteTask", "ReopenTask", "CancelTask", "MoveTask")
    ...
    fun project(task: JSONObject, intent: SharedIntent, intents: List<SharedIntent>, applied: Set<String>): String? {
        ...
        for (group in observedGroups(intent.kind)) {
            ...
            if (version(task, group) != expected) return when (group) {
                "claim" -> "CLAIM_CONFLICT"; "orderIntent" -> "ORDER_CONFLICT"; else -> "LIFECYCLE_CONFLICT"
            }
        }
        ...
        if (receipt != null) {
            ...
            for (key in listOf("lifecycle", "claimantId", "lifecycleActorId", "lifecycleAt", "firstCompletion"))
    ...
    fun ordered(rows: List<SharedProjection>, intents: List<SharedIntent>, state: SharedWorkspace?): List<JSONObject> {
        ...
        val active = tasks.filterValues { it.optString("lifecycle", "OPEN") == "OPEN" }
        ...
        val inserted = queued.filter { it.kind == "CreateTask" || it.kind == "ReopenTask" }.map { it.taskId }.toSet()
        ...
        for (intent in queued) {
            if (intent.kind in listOf("CompleteTask", "CancelTask")) { order.remove(intent.taskId); continue }
            if (intent.kind == "CreateTask" || intent.kind == "ReopenTask" &&
