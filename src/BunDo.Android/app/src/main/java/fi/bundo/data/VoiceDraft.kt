...
data class VoiceDraft(val transcript: String, val title: String, val description: String = "", val items: List<String> = emptyList()) {
    ...
    fun json(): String = JSONObject().put("transcript", transcript).put("title", title)
        .put("description", description).put("items", JSONArray(items)).toString()
    companion object {
