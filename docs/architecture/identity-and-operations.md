...
Offer a user-initiated UTF-8 JSON and plain-text recovery export through Android's document picker. Include local draft text and unresolved variants, with workspace labels and capture times. Exclude credentials, invitation secrets and reusable device identity. Import is an explicit preview that creates new task IDs; it never replays exported commands. Warn that exported files contain household text. Raw audio export is a separate explicit action in voice settings. Since the September 16, 2026 owner revision, new recordings default to temporary audio only. Persist the transcript as a review draft before deleting audio. Delete unretained audio after failure/cancel and during restart recovery. Keeping recordings is an explicit device preference for future captures, including successful ones, with seven-day expiry and bounded storage. Review text does not expire with audio. Existing recovery recordings retain their previous expiry on upgrade. Include voice-review text in same-account text recovery; normal onboarding has no recording import step.
...
Keep the deployed backup configuration and verify it as part of the [v1 operations slice](https://github.com/DrBushyTop/bun-do/issues/50). Follow the [manual recovery procedure](../manual-recovery.md) for isolated restore with traffic/workers stopped, integrity checks, a fresh external epoch, invalidated old registrations/invitations, owner membership review and explicit client comparison/import before resuming. Restored AI work cannot run automatically under stale authority. Old commands never replay automatically across the epoch change.
